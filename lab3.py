...
if df_base is not None:
    ...
    # --- 🗓️ CALENDARI (LOGICA ULTRA-ROBUSTA) ---
        ...
        if not files_cal:
            ...
            try:
                ...
                if not giornate_trovate:
                    ...
                    # Identifica se è formato Champions (colonna Gruppo presente)
                    is_coppa = df_cal.shape[1] > 10
                    offset = 1 if is_coppa else 0
                    
                    # Prende tutte le istanze della giornata selezionata (pari e dispari)
                    istante_giornata = [g for g in giornate_trovate if g["nome"] == sel_g]
                    
                    for pos in istante_giornata:
                        r_start, c_start = pos["r"], pos["c"]
                        # Analizza le 12 righe successive
                        for i in range(1, 13):
                            if r_start + i < len(df_cal):
                                row = df_cal.iloc[r_start + i]
                                # Se troviamo un'altra intestazione di giornata, fermiamoci
                                if "giornata" in str(row[c_start]).lower(): break
                                
                                val_check = str(row[c_start + offset]).strip()
                                if "riposa" in val_check.lower():
                                    riposi.append(val_check)
                                    continue
                                
                                try:
                                    h = str(row[c_start + offset]).strip()
                                    a = str(row[c_start + offset + 3]).strip()
                                    if not h or h == "" or a == "" or h == "nan": continue
                                    
                                    # Pulizia punteggi
                                    sh = str(row[c_start + offset + 1]).replace(',','.').replace('"','').strip()
                                    sa = str(row[c_start + offset + 2]).replace(',','.').replace('"','').strip()
                    ...
                    if match_list:
                        ...
                        st.info("Partite già giocate o nessuna partita trovata.")
                    if riposi:
    ...
    # --- RESTANTI SEZIONI (DASHBOARD, MERCATO, ECC.) ---
    elif menu == "🏠 Dashboard":
