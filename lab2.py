...
# --- AUTO-DIAGNOSI DEI FILE ---
st.sidebar.title("🔍 Stato File")
files_nella_cartella = os.listdir('.')
file_necessari = {
    'fantamanager-2021-rosters.csv': 'Rose',
    'quot.csv': 'Quotazioni',
    'leghe.csv': 'Database Leghe',
    'stadi.csv': 'Database Stadi',
    'Calendario_PREMIER-LEAGUE.csv': 'Calendario'
}

for filename, label in file_necessari.items():
    if filename in files_nella_cartella:
...
MAPPATURA_COLORI = {
    "Serie A": "#00529b", "Bundesliga": "#d3010c", 
    "Premier League": "#3d195b", "Liga BBVA": "#ee8707"
}
...
if df_base is not None:
    ...
    menu = st.sidebar.radio("Vai a:", [
        "🏠 Dashboard", "📅 Calendario Premier", "🏟️ Calcolo Rapido Stadio", 
        "🏃 Svincolati *", "✂️ Tagli Volontari", "📊 Ranking FVM", 
        "📋 Visualizza Rose", "🟢 Giocatori Liberi", "⚙️ Gestione Squadre"
    ])
        ...
        for i, nome_lega in enumerate(leghe_effettive):
            with cols[i % 2]:
                ...
                for _, sq in tabella.sort_values(by='Squadra_LFM').iterrows():
                    ...
                    colore_count = "#81c784" if 25 <= n_g <= 35 else "#ef5350"
                    st.markdown(f"""<div style="background-color: {bg_color}; padding: 15px; border-radius: 12px; margin-bottom: 12px; color: white; border: 1px solid rgba(255,255,255,0.2);">
                        <div style="display: flex; justify-content: space-between;"><b>{sq['Squadra_LFM']}</b> <span style="background:{colore_count}; padding:2px 8px; border-radius:10px; font-size:11px;">🏃 {n_g}/25-35</span></div>
