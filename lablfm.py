...
def load_static_data():
    for enc in ['latin1', 'cp1252', 'utf-8']:
        try:
            ...
            # CALCOLO TAGLI VOLONTARI (50%: (FVM + Qt.I)/2)
...
if df_static is not None:
        ...
        for i, nome_lega in enumerate(leghe_effettive):
            with cols[i % 2]:
                ...
                for _, sq in tabella.sort_values(by='Squadra_LFM').iterrows():
                    d_html = ""
                    if sq['Nome']: d_html += f"<div style='font-size:13px;color:#d32f2f;'><b>* Svinc:</b> {sq['Nome']} (+{int(sq['Rimborso_Star'])})</div>"
    ...
    # --- 🏃 SVINCOLATI * (Globali) ---
        ...
        st.title("🏃 Rimborsi da * (Rimborso Pieno)")
    ...
    # --- 📊 RANKING FVM ---
        ...
        pivot_rank = df_rank.pivot_table(index=['FVM', 'Nome', 'R'], columns='Lega', values='Squadra_LFM', aggfunc=lambda x: ", ".join(x)).reset_index()
