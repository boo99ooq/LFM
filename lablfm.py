...
def load_static_data():
    for enc in ['latin1', 'cp1252', 'utf-8']:
        try:
            ...
            df['Nome'] = df['Nome'].fillna("ID: " + df['Id'].astype(str))
...
if df_static is not None:
    ...
    # Chiave per tagli selettivi
    def get_taglio_key(row):
        if pd.isna(row['Squadra_LFM']): return "LIBERO"
        return f"{int(row['Id'])}_{row['Squadra_LFM']}"
    
    df_base['Taglio_Key'] = df_base.apply(get_taglio_key, axis=1)
    ...
    # --- 🏃 SVINCOLATI * (Solo chi ha squadra) ---
    elif menu == "🏃 Svincolati *":
        st.title("✈️ Rimborsi da *")
        df_svinc = df_base.dropna(subset=['Squadra_LFM'])
        cerca = st.text_input("Cerca giocatore:", key="c_star")
        if cerca:
            df_f = df_svinc[df_svinc['Nome'].str.contains(cerca, case=False, na=False)].drop_duplicates('Id')
            ...
            if st.button("Salva"):
                ...
                st.rerun()
        st.divider()
        st.dataframe(df_svinc[df_svinc['Rimborsato_Star']].drop_duplicates('Id')[['Nome', 'R', 'Qt.I', 'FVM', 'Rimborso_Star']], use_container_width=True, hide_index=True)

    # --- ✂️ TAGLI VOLONTARI (Solo chi ha squadra) ---
    elif menu == "✂️ Tagli Volontari":
        st.title("✂️ Tagli Volontari")
        df_tagli = df_base.dropna(subset=['Squadra_LFM'])
        cerca_t = st.text_input("Cerca per squadra:", key="c_tag")
        if cerca_t:
            df_t = df_tagli[df_tagli['Nome'].str.contains(cerca_t, case=False, na=False)]
            edit_t = st.data_editor(df_t[['Rimborsato_Taglio', 'Nome', 'R', 'FVM', 'Qt.I', 'Squadra_LFM', 'Rimborso_Taglio', 'Taglio_Key']], hide_index=True, use_container_width=True)
    ...
    # --- 📊 RANKING FVM (Include Liberi) ---
        ...
        pivot = df_rank.pivot_table(index=['FVM', 'Nome', 'R', 'Id'], columns='Lega', values='Display', aggfunc=lambda x: " | ".join(str(v) for v in x if str(v) != 'nan'), dropna=False).reset_index()
    ...
    # --- 📋 VISUALIZZA ROSE ---
        ...
        df_rose_v = df_base.dropna(subset=['Squadra_LFM'])
        l_sel = st.selectbox("Lega:", sorted(df_rose_v['Lega'].unique()))
        s_sel = st.selectbox("Squadra:", sorted(df_rose_v[df_rose_v['Lega'] == l_sel]['Squadra_LFM'].unique()))
        df_res = df_rose_v[df_rose_v['Squadra_LFM'] == s_sel].copy()
        st.dataframe(df_res[['Nome', 'R', 'Qt.I', 'FVM']], use_container_width=True, hide_index=True)
...
else: st.error("Carica i file CSV!")mport streamlit as st
import pandas as pd
